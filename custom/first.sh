...
if [[ -n "${BENDI_VERSION}" ]]; then
  ...
  sudo rm -rf build/common && git clone -b main --depth 1 https://github.com/dream-snow/common build/common
...
git clone -b main --depth 1 https://github.com/dream-snow/build-actions shangyou
...
if [[ "${GIT_REPOSITORY}" =~ (dream-snow/build-actions|dream-snow/auto-build) ]]; then
  rm -rf backups
...
curl -fsSL https://raw.githubusercontent.com/dream-snow/common/main/common.sh -o common.sh
if [[ $? -ne 0 ]]; then
  wget -q https://raw.githubusercontent.com/dream-snow/common/main/common.sh -O common.sh
fi
...
if [[ -n "${BENDI_VERSION}" ]]; then
  ...
  export GIT_REPOSITORY="dream-snow/build-actions"
  sudo rm -rf build common.sh
